...
public class Prospector : MonoBehaviour {
	...
	static public int				SCORE_FROM_REV_ROUND = 0;
	...
	public Vector3					fsPosEnd = new Vector3 (0.5f, 0.65f, 0);
	...
	void Awake () {
		...
		score += SCORE_FROM_REV_ROUND;

		// And reset the score from previous round
		SCORE_FROM_REV_ROUND = 0;
	...
	void Start () {
		...
		deck.InitDeck (deckXML.text);		// Pass DeckXMLto it
	...
	// LayoutGame() positions the initial tanleau of cads aka the "mine"
	void LayoutGame() {
		// Create an empty GameObject to serve as an anchor or the tableau	//1
		if (layoutAnchor == null) {
			GameObject tGO = new GameObject ("_LayoutAnchor");
			// ^ Create an empy GameObject names _LAyoutAnchor in the Hieratchy
		...
		foreach (SlotDef tSD in layout.slotDefs) {
			// ^ Iterate through all the SLotDegs in the layout,slotDefs as tSD
			cp = Draw ();		// Pull a card from the top of the drawPile
			cp.faceUp = tSD.faceUp;	// Set its faceUp to the value in SlotDef
			cp.transform.parent = layoutAnchor; // Make its parent layoutAnchor

			// This replaces the previous parent: deck.deckAnchor, which appears as 
			// _Deck in the Hierarchy when the scene is playing
	...
	// Card Clicked is called any time a card in the game is clicked
	public void CardClicked(CardProspector cd) {
		...
		case CardState.drawpile:
			...
			UpdateDrawPile ();			// Restacjs the drawPile
		...
		case CardState.tableau:
			...
			if (!AdjacentRank (cd, target)) {
				// If it si not an adjacent rank, it is not valid
				validMatch = false;
			}
	...
	// Moves the current target to the discard pile
	void MoveToDiscard(CardProspector cd) {
		...
		cd.transform.localPosition = new Vector3 (
			layout.multiplier.x * layout.multiplier.x,
	...
	void UpdateDrawPile() {
		...
		for (int i = 0; i < drawPile.Count; i++) {
			...
			cd.faceUp = false;	// Make them all facedown
	...
	// Return true if the two cards a adjacent in rank (A and K wrap around)
		...
		foreach(CardProspector cd in tableau) {
			...
			foreach (CardProspector cover in cd.hiddenBy) {
				...
				if (cover.state == CardState.tableau) {
					fup = false;	// Then the card is face-down
				}
			}
	...
	// Called when the game is over. Simple for now, but expendable
	void GameOver(bool won) {
		...
		// r=Reload the game in reloadDelay seconds
	...
	void ReloadLevel() {
		// Releaod thscene resetting the game
		Application.LoadLevel ("_Prospector_Scene_0");
	}

	// ScoreManager handles the scoring
	void ScoreManager (ScoreEvent sEvt) {
		List<Vector3> fsPts;

		switch (sEvt) {
		// Same things need to happen whether its a drwa, a win or a loss
		...
		case ScoreEvent.gameLoss:	// Lost the round
			...
			if (fsRun != null) {
				...
				fsRun.fontSizes = new List<float> (new float[] {28, 36, 5});
		...
		case ScoreEvent.gameWin:
			...
			// statinc fields are NOT reset by Application.LoadLevel()
			Prospector.SCORE_FROM_REV_ROUND = score;
